...
private:
  ...
  void getInterpolants (double t, double &t0, double &t1, VectorType &p0,
			VectorType &m0, VectorType &p1, VectorType &m1);
};

template <typename VectorType>
inline bool SplineInterpolator<VectorType>::generateFromCSV
(const char* filename) {
  std::ifstream infile (filename);
  if (!infile) {
    std::cerr << "Error spline input file from '" << filename << "'"
	      << std::endl;
    abort();
    return false;
  }
  ...
  while (!infile.eof()) {
	...
	std::cerr << "Error: could not convert std::string '" << tokens[i]
		  << "' to number in " << filename << ", line "
		  << line_index << ", column " << i << std::endl;
	abort();
      }
...
template <typename VectorType>
void SplineInterpolator<VectorType>::addPoints (double t,
...
inline void SplineInterpolator<VectorType>::initialize() {
  ...
  m_values = std::vector<VectorType>(p_values.size(),
				     VectorType::Zero(p_values[0].size()));

  m_values[0] = (p_values[1] - p_values[0]) / (t_values[1] - t_values[0]);

  size_t last_index = t_values.size() - 1;
  m_values[last_index] = (p_values[last_index] - p_values[last_index - 1]) /
    (t_values[last_index] - t_values[last_index - 1]);

  for (size_t i = 1; i < t_values.size() - 1; i ++) {
    VectorType m_value = VectorType::Zero(p_values[0].size());

    // Catmull-Rom Spline
    m_value = (p_values[i + 1] - p_values[i - 1]) /
      (t_values[i + 1] - t_values[i - 1]);
    m_values[i] = m_value;
  }
...
template <typename VectorType>
inline void SplineInterpolator<VectorType>::getInterpolants
(double t, double &t0, double &t1, VectorType &p0, VectorType &m0,
 VectorType &p1, VectorType &m1) {
  ...
  std::cerr << "Could not find interpolants at time " << t << ". Range is ["
	    << t_values[0] << ", " << t_values[t_values.size() - 1] << "]"
	    << std::endl;
  abort();
}
...
inline VectorType SplineInterpolator<VectorType>::getValues(double t) {
  ...
  return (h00(tau) * p0 + h10(tau) *
	  (t1 - t0) * m0 + h01(tau) * p1 + h11(tau) * (t1 - t0) * m1);
}
...
inline VectorType SplineInterpolator<VectorType>::getDerivatives(double t) {
  ...
  return (h00_dot(tau) * taudot * p0 + h10_dot(tau) *
	  taudot * (t1 - t0) * m0 + h01_dot(tau) * taudot *
	  p1 + h11_dot(tau) * taudot * (t1 - t0) * m1);
}

template <typename VectorType>
inline VectorType SplineInterpolator<VectorType>::getSecondDerivatives
(double t) {
  initialize();
  ...
  return (h00_ddot(tau) * taudot_2 * p0 + h10_ddot(tau) *
	  taudot_2 * (t1 - t0) * m0 + h01_ddot(tau) * taudot_2 *
	  p1 + h11_ddot(tau) * taudot_2 * (t1 - t0) * m1);
}
